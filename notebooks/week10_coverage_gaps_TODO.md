...
- [ ] **[P1] Braid the delivered MG W10 content, not next week's.** The single biggest gap. The genuine, low-cost contact point is the deck's own §G/§42/§43/§48: *transformer-style attention applied to crystals (Matformer/Graphormer), the pretrain→freeze→fine-tune foundation-model recipe, and "Unit 9 produces the embedding."* Block 5 (frozen ViT-Ising → Cahn-Hilliard) is already a faithful analogue of the §43 recipe — add a short markdown bridge explicitly naming Matformer / OMat24 / MACE-MP-0 and stating "this frozen-encoder→new-head transfer is exactly the §43 materials foundation-model recipe, on images instead of crystal graphs." Cheap, and it makes the MG braid real instead of mislabeled.
- [ ] **[P2] Sum (extensive) vs mean (intensive) readout.** [CORE] in the delivered deck (slide 11, "choosing the wrong pooling silently breaks transferability across cell sizes"). `TinyViT` only ever does CLS-token readout; Exercise 3 introduces mean-pooling but never the extensive/intensive *reasoning*. Extend Exercise 3 (or add a block) contrasting sum-pool vs mean-pool and stating the extensive (total energy) vs intensive (band gap / per-atom) distinction — a clean, in-budget MG braid that needs no GNN.
- [ ] **[P2] Permutation-equivariance as the *materials* symmetry, made explicit.** [CORE]: the deck's "four symmetries" (translation, rotation, permutation, periodicity) and "permutation invariance via sum/mean aggregation" are the spine of MG W10. Block 2 already demonstrates permutation-equivariance of attention beautifully — add two sentences connecting it to the MG framing ("attention is permutation-equivariant by default; for an unordered atom set that is the *correct* symmetry, exactly why GNN message passing uses sum/mean; for an ordered patch grid it is a bug fixed by PE"). Reuses an existing block; closes the symmetry thread for free.
- [ ] **[P3] Invariance vs equivariance (scalars vs forces/tensors).** [CORE] conceptual spine of the delivered deck, but genuinely out of scope for an image-ViT notebook (no forces, no 3D structures). Optionally add one markdown cell stating the rule (invariant readout for scalars; equivariant features for forces/dipoles; autograd-forces are correct but data-hungry vs native-equivariant NequIP/MACE) as a forward pointer, not an experiment.
- [ ] **[P3] The named atomic-system architectures (SchNet/CGCNN/MEGNet/ALIGNN/M3GNet/NequIP/MACE) and the four-symmetry / continuous-filter machinery.** [CORE] in the delivered deck but architecturally disjoint from attention-on-images. Not worth re-implementing in this notebook; the right closure is the [P1] bridge above plus a one-line reading pointer to the MG W10 deck's CGCNN exercise. Flagged for completeness, not as an action.
