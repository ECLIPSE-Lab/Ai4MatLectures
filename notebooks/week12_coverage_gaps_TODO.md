...
- [x] **[P1] Generative model for crystals/structures (the entire MG U12 spine).** [CORE] **CLOSED** — Block 6.5 Stage A is a conditional VAE (the deck's legacy "VAE row") making `sample from p(x | y*)` concrete; Stage B runs the same pipeline on the **real CDVAE perovskite benchmark** (`CDVAEMaterialsDataset`, perov_5, 118-dim composition), the exact dataset family the CDVAE→DiffCSP→MatterGen→FlowMM lineage uses.
- [x] **[P1] Conditional / inverse-design loop (sample for a target property).** [CORE] **CLOSED** — Block 6.5 Stage A trains a CVAE with classifier-free guidance and generates candidates conditioned on a target hardness H*; Stage B targets a runtime-discovered real DFT property column on real crystals. Braids directly with the MFML UQ surrogate.
- [x] **[P2] The discovery funnel + S.U.N. (Stable/Unique/Novel) evaluation.** [CORE] **CLOSED** — Block 6.5 implements the full generate→validity→uniqueness→novelty→on-target→uncertainty-triage funnel with a reported S.U.N.-style rate, on both the 2-D toy (Stage A) and real perov_5 crystals (Stage B, non-degenerate waterfall 8000→1522, S.U.N. 61.5%, yield 19.0%). Homework Part E (1-D) + Part E+ (real-crystal scaffold) mirror it.
- [x] **[P3] Uncertainty-aware filtering as the generative↔UQ bridge.** [SUPPORT] **CLOSED** — the triage stage reuses the Block-5 per-cluster GP predictive variance (Stage A) and a random-forest ensemble cross-tree variance (Stage B) to reject generated candidates the surrogate cannot vouch for: generation proposes, uncertainty disposes.
