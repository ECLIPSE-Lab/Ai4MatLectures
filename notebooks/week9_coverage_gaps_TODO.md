...
- [x] **[P1] Re-frame the MG block to the actual W9 lecture (regression & generalization), not U9/W10 architectures.** DONE — Blocks 4–5 are now an explicit MG-W9 *trustworthy regression* arc on the **real `MatBenchDataset` perovskites benchmark** (`ds.X`/`ds.y`); the TinyCGNN survives only as a clearly-labelled *synthetic structural appendix* (Block 5b). Headers re-titled accordingly; homework Part E mirrors at homework depth.
- [x] **[P1] Split-design taxonomy: random vs group-aware.** DONE — Block 4 / Part E.1 contrast random 80/20 vs the dataset's reproducible *surrogate* `ds.folds` CV vs a composition-family hold-out (dominant non-O element Z∈{3,4,5}), reporting the family − random gap as $\Delta_{\text{shift}}$.
- [x] **[P1] The mandatory baseline ladder.** DONE — tiers 0/1/2 (constant / ridge / GBT) on the real composition vector, train-only standardiser, same split machinery; runs clean.
- [x] **[P1] Polymorph aliasing / structure-awareness ablation.** DONE — kept on the toy `CrystalGraphsDataset` in the Block-5b synthetic appendix (edge-distance scramble + inflation factor); N/A on the composition-only MatBench benchmark, stated explicitly in the checklist.
- [x] **[P2] Per-region residual diagnostics.** DONE — per-composition-family MAE + signed bias table on the MatBench random-split residuals (Block 5 / Part E.2).
- [x] **[P2] OOD-vs-interpolation distance diagnostic.** DONE — Block 5 bins the family-held-out test points by min composition-space distance to train and reports MAE per quartile + the extrapolation-tail scatter.
- [x] **[P2] Confidence interval on the headline metric.** DONE — Block 5 bootstrap 95% CI (2000 resamples) on the headline MatBench MAE.
- [ ] **[P3] Leakage audit demo (preprocessing-before-split / dedup).** [CORE] §D32/§F: standard-scaler-fit-on-full-data, polymorph leakage. Optional but high-pedagogy: a deliberate "fit normalization on full data then split" vs "fit on train only" contrast showing the inflated test metric — the deck's most concrete leakage anti-pattern. (Partially covered: every tier already fits the standardiser on train only and says so; the deliberate *wrong-way* contrast is still unbuilt.)
- [x] **[P3] The 7-point trustworthy-reporting checklist as the block's closing rubric.** DONE — closing scored 7-point checklist in Block 5 (real MatBench part, 6/7) and Part E.3.
